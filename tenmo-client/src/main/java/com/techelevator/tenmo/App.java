...
public class App {
	...
	private void viewTransferHistory() {
		int accountId = 0;
		accountId = restTemplate.exchange(API_BASE_URL + "accounts/" + App.ID, HttpMethod.GET, makeAuthEntity(), int.class).getBody();
	...
	private boolean viewPendingRequests() {
		...
		try {
			...
			if (approveOrDeny == 1) {
				...
				String username = "";
				username = restTemplate.exchange(API_BASE_URL + "users/account/" + selectedTransfer.getAccount_to(), HttpMethod.GET, makeAuthEntity(), String.class).getBody();
				...
				requestedUserId = restTemplate.exchange(API_BASE_URL + "users/userId/" + selectedTransfer.getAccount_to(), HttpMethod.GET, makeAuthEntity(), int.class).getBody();
	...
	private boolean sendBucks() {
		...
		console.printUsers(userService.listAllUsers(App.ID));  
		...
		success = userService.validateTransfer(idSendingTo, amountBD, App.USERNAME, App.ID);
		if (success) {
	...
	private boolean requestBucks() {
		...
		try {
			transferService.createRequestTransfer(idSendingTo, App.ID, amountBD);
		} 
		catch (Exception e) {
			System.out.println("\nInvalid Selection!");
		}
