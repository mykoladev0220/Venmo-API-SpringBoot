...
private static final String API_BASE_URL = "http://localhost:8080/";
	...
	private boolean viewPendingRequests() {
		...
		if (approveOrDeny == 1) {
			...
			if (enoughMoney) {
				transferService.increaseBalance(requestedUserId, selectedTransfer.getAmount());
				transferService.decreaseBalance(App.ID, selectedTransfer.getAmount());
				System.out.println("\nTransfer accepted!");
	...
	private boolean sendBucks() {
		...
		if (success) {
			...
			transferService.decreaseBalance(App.ID, amountBD);
			transferService.increaseBalance(idSendingTo, amountBD);
		}
		return true;
	}
